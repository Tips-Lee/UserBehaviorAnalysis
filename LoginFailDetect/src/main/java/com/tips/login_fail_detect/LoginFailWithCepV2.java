...
public class LoginFailWithCepV2 {
    public static void main(String[] args) throws Exception{
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(1);
        env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime);
        String inputPath = "/LoginLog.csv";
        final URL resource = LoginFailWithCepV2.class.getResource(inputPath);
        ...
        // 1. 创建pattern
        Pattern<LoginEvent, LoginEvent> pattern = Pattern.<LoginEvent>begin("failEvent").where(new SimpleCondition<>() {
            @Override
        ...
        SingleOutputStreamOperator<LoginFailWarning> outputStream = patternStream.select(new PatternSelectFunction<LoginEvent, LoginFailWarning>() {
            ...
            public LoginFailWarning select(Map<String, List<LoginEvent>> map) {
                List<LoginEvent> failEvent = map.get("failEvent");
                LoginEvent firstFail = failEvent.iterator().next();
                LoginEvent lastFail = failEvent.get(failEvent.size()-1);
                return new LoginFailWarning(firstFail.getUserId(), firstFail.getTimestamp(), lastFail.getTimestamp(), "login failed 3 times");
            }
        });
